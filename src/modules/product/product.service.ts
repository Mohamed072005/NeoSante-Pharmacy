...
export class ProductService implements ProductServiceInterface {
  ...
  async handleCreateProduct(productDto: ProductDto,): Promise<{ message: string }> {
    ...
    const product = await this.productRepository.findProductByPharmacyIdAndBarcodeAndName(pharmacyObjectId, productDto.barcode, productDto.name,);
    if (!product) {
      throw new BadRequestException(
        'You already have a product with this name or barcode',
      );
    }
